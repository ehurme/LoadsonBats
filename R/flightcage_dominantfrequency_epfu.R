...
for(i in 1:length(files)){
  ...
  df$burstCount %>% as.numeric() %>% table
  # add tag time
...
psych::pairs.panels(fc_freq)
fit_tagweight <- glm(frequency ~ tag_weight + trial, data = fc_freq)
fit_totalweight <- glm(frequency ~ total_weight + trial, data = fc_freq)
summary(fit_totalweight)

save(fc_freq, sampling_rate, window_length,
     file = paste0("../../../Dropbox/MPI/Wingbeat/Arizona/flightcage/epfu_wingbeat_wl_", window_length, ".robj"))
